...
class ExpSetup:
    ...
    def __init__(self, datasets_parent_dir=None, task_str=None, logdir=None, load_specific_ckpt=None):
        ...
        task_str = "swfd_ss64,sc" if task_str is None else task_str
        ...
        ###############################
        self.hpobj = utils.ImageTranslationSetups(task_str=task_str, datasets_parent_dir=self.datasets_parent_dir)
        ...
        self.mapping_out_tf = self.mapping_in_tf
        scaleclip_fn = lambda x: np.clip(x/np.max(x), a_min=-0.2, a_max=None)
        # self.mapping_in = lambda x: np.expand_dims(scaleclip_fn(x), axis=-1)
        self.mapping_in = None
        self.mapping_out = self.mapping_in
        ...
        self.model = architectures.modified_unet(in_shape=in_shape, out_activations=self.out_activation, out_channels=1, l1reg=self.l1_reg, l2reg=self.l2_reg)
        self.model.summary()
    ...
    def setup_datasets(self):
        ...
        if isinstance(self.in_key, list): ## need to an individual generator for each self.in_key & out_key pair, then merge. This ONLY applies to MSFD
            if len(self.in_key) != len(self.out_key):  
                raise AssertionError('Script expects that a list for in_key should have a matching list of out_key')
            l_gen = []
            for i in range(len(self.in_key)):
                in_key = self.in_key[i]
                out_key = self.out_key[i]
                gen = generators.Generator_Paired_Input_Output(fname_h5=self.fname_h5, inds=inds, in_key=in_key, out_key=out_key, transforms=transforms, transforms_target=transforms_target, prng=self.prng, shuffle=True)
                l_gen.append(gen)
            self.gen_train = generators.Generator_Group_Sampler(l_gens=l_gen)
        else:
        ...
        if isinstance(self.in_key, list): ## need to an individual generator for each self.in_key & out_key pair, then merge. This ONLY applies to MSFD
            ...
            for i in range(len(self.in_key)):
                ...
                gen = generators.Generator_Paired_Input_Output(fname_h5=self.fname_h5, inds=inds, in_key=in_key, out_key=out_key, transforms=transforms, transforms_target=transforms_target, prng=self.prng, shuffle=True)
                l_gen.append(gen)
            self.gen_val = generators.Generator_Group_Sampler(l_gens=l_gen)
        else:
    ...
    def serialize_with_best_val_on_exit(self):
        fl_best_val = glob.glob(os.path.join(self.logdir, 'model_best_val_loss_ckpt-*'))
        ckpt_numbers = [int(os.path.basename(fname)[len('model_best_val_loss_ckpt-'):].split('.')[0]) for fname in fl_best_val]
        ind_ckpt_best = np.argmax(ckpt_numbers)
        ckpt_num = ckpt_numbers[ind_ckpt_best]
        fname_ckpt = fl_best_val[ind_ckpt_best]
        fname_serialized =  os.path.join(self.logdir, 'serialized_model_step_%d'%ckpt_num)
        self.setup_checkpoint_manager(load_specific_ckpt=os.path.splitext(os.path.basename(fname_ckpt))[0])
        model = self.model
        model.save(fname_serialized)
        logging.info(f'Model state at best validation loss (step: {ckpt_num}) is saved as {fname_serialized}.')
...
def train(args):
    ...
    def eval_and_log(model, dataset, loss_fn, filter_size_ssim, log=True, writer=None, num_step=None):
        ...
        for x, y in dataset:
            ...
            for i in range(1): # there's a single out channel
                if 'RMSE_ch_%d'%(i+1) not in d_rmse:
                    d_rmse['RMSE_ch_%d'%(i+1)] = tf.keras.metrics.RootMeanSquaredError(name='rmse_eval', dtype=tf.float32)
                d_rmse['RMSE_ch_%d'%(i+1)].update_state(y[i], y_pred[i])
                if 'MAE_ch_%d'%(i+1) not in d_mae:
                    d_mae['MAE_ch_%d'%(i+1)] = tf.keras.metrics.MeanAbsoluteError(name='mae_eval', dtype=tf.float32)
                d_mae['MAE_ch_%d'%(i+1)].update_state(y[i], y_pred[i])
                if 'SSIM_ch_%d'%(i+1) not in d_ssim:
                    d_ssim['SSIM_ch_%d'%(i+1)] = tf.keras.metrics.Mean(name='ssim_eval', dtype=tf.float32)
                d_ssim['SSIM_ch_%d'%(i+1)].update_state(tf.math.reduce_mean(tf_utils.ssim_fn(tf_utils.im_norm(y[i]), tf_utils.im_norm(y_pred[i]), filter_size_ssim=filter_size_ssim)))
                if 'SSIM_MS_ch_%d'%(i+1) not in d_ssim_ms:
                    d_ssim_ms['SSIM_MS_ch_%d'%(i+1)] = tf.keras.metrics.Mean(name='ssim_ms_eval', dtype=tf.float32)
                d_ssim_ms['SSIM_MS_ch_%d'%(i+1)].update_state(tf.math.reduce_mean(tf_utils.ssim_ms_fn(tf_utils.im_norm(y[i]), tf_utils.im_norm(y_pred[i]), filter_size_ssim=filter_size_ssim)))
        if writer is not None:
            assert (num_step is not None) # num_step must be provided.
            d_scalars = {'loss': avg_loss.result()}
            for k in d_rmse.keys():
                d_scalars[k] = d_rmse[k].result()
            for k in d_mae.keys():
                d_scalars[k] = d_mae[k].result()
            for k in d_ssim.keys():
                d_scalars[k] = d_ssim[k].result()
            for k in d_ssim_ms.keys():
                d_scalars[k] = d_ssim_ms[k].result()
            ...
            for i in range(1):
                d_images['y_true_ch_%d'%(i+1)] = tf_utils.im_norm(y[0:1, ...])
                d_images['y_pred_ch_%d'%(i+1)] = tf_utils.im_norm(y_pred[0:1, ...])
    ...
    for epoch in range(num_epoch, num_epochs_train):
        ...
        for x,y in dataset_train: # loop over training step for a full epoch
            ...
            for i in range(1):
                if 'RMSE_ch_%d'%(i+1) not in d_rmse:
                    d_rmse['RMSE_ch_%d'%(i+1)] = tf.keras.metrics.RootMeanSquaredError(name='rmse_training', dtype=tf.float32)
                d_rmse['RMSE_ch_%d'%(i+1)].update_state(y[i], y_pred[i])
                if 'MAE_ch_%d'%(i+1) not in d_mae:
                    d_mae['MAE_ch_%d'%(i+1)] = tf.keras.metrics.MeanAbsoluteError(name='mae_training', dtype=tf.float32)
                d_mae['MAE_ch_%d'%(i+1)].update_state(y[i], y_pred[i])
                if 'SSIM_ch_%d'%(i+1) not in d_ssim:
                    d_ssim['SSIM_ch_%d'%(i+1)] = tf.keras.metrics.Mean(name='ssim_training', dtype=tf.float32)
                d_ssim['SSIM_ch_%d'%(i+1)].update_state(tf.math.reduce_mean(tf_utils.ssim_fn(tf_utils.im_norm(y[i]), tf_utils.im_norm(y_pred[i]), filter_size_ssim=filter_size_ssim)))
                if 'SSIM_MS_ch_%d'%(i+1) not in d_ssim_ms:
                    d_ssim_ms['SSIM_MS_ch_%d'%(i+1)] = tf.keras.metrics.Mean(name='ssim_ms_training', dtype=tf.float32)
                d_ssim_ms['SSIM_MS_ch_%d'%(i+1)].update_state(tf.math.reduce_mean(tf_utils.ssim_ms_fn(tf_utils.im_norm(y[i]), tf_utils.im_norm(y_pred[i]), filter_size_ssim=filter_size_ssim)))
            ...
            if tf.equal(optimizer.iterations % freq_eval_val, 0):
                d_scalars = eval_and_log(model=model, dataset=dataset_val, loss_fn=loss_fn, filter_size_ssim=filter_size_ssim, log=True, writer=summary_writer_val, num_step=optimizer.iterations)
    ...
    args.serialize_with_best_val_on_exit()
